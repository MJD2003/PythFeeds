...
function logFatal(where, err) {
  ...
  const stack =
    err && typeof err === "object" && "stack" in err ? err.stack : String(err);
  console.error(`[${ts}] [FATAL] ${where}\n${stack}`);
}
...
// Comma-separated: https://pythfeeds.com,https://www.pythfeeds.com,http://localhost:3000
...
const server = app.listen(PORT, () => {
  ...
  // Cache warm-up: stagger CoinGecko calls to avoid 429s (free tier ~10 calls/min)
  setTimeout(async () => {
    console.log("[Warm-up] Pre-fetching market data (staggered)...");
  ...
  // Generate first digest 2 min after startup (after warm-up finishes), then every 2 hours
  setTimeout(autoGenerateDigest, 120000);
  setInterval(autoGenerateDigest, 2 * 60 * 60 * 1000);
});

server.on("error", (err) => {
  logFatal("HTTP server listen error", err);
  if (err && err.code === "EADDRINUSE") {
    console.error(`[FATAL] Port ${PORT} is already in use (another process?)`);
  }
  process.exit(1);
});
